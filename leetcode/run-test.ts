...
const runTestCase = <T, K>(fn: (args: K) => T, fnArgs: K, expected: T) => {
    const res = fn(fnArgs);
    if (isEqual(res, expected)) {
        console.log(bgGreen(black(" PASS! ")), "Expected: ", expected, "Output: ", res, "Fn Args: ", fnArgs);
    } else {
        console.log(bgRed(black(" FAIL! ")), "Expected: ", expected, "Output: ", res, "Fn Args: ", fnArgs);
    }
};
